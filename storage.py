...
def draw_wardrobe():
    ...
    # ── Mahkota atas — flush dengan puncak badan ──────
    # Plat tipis tepat di puncak (tidak mencuat keluar)
    mat_wood(0.50, 0.36, 0.18)
    draw_box(wx - 0.01, wH - 0.06, wz, wW + 0.04, 0.06, wD + 0.04,
             0.50, 0.36, 0.18)
    # Strip profil depan — tidak melebihi wH
    mat_wood(0.34, 0.22, 0.10)
    draw_box(wx - 0.25, wH - 0.07, wz, 0.04, 0.07, wD + 0.02,
             0.34, 0.22, 0.10)
...
def _draw_wardrobe_door(wx, dz, wH):
    ...
    # Stile mengisi ruang antara rail bawah dan rail atas
    stile_bot = 0.09 + 0.07      # tepat di atas rail bawah
    stile_top = fw - 0.04        # tepat di bawah rail atas
    stile_h   = stile_top - stile_bot  # tinggi stile yang benar
    mat_wood(0.46, 0.32, 0.15)
    draw_box(px + 0.002, stile_bot, dz - fd/2 + 0.04,
             0.055, stile_h, 0.06, 0.46, 0.32, 0.15)
    draw_box(px + 0.002, stile_bot, dz + fd/2 - 0.04,
             0.055, stile_h, 0.06, 0.46, 0.32, 0.15)

    # Panel kayu bawah (30% bawah pintu)
    panel_bot_h = fw * 0.28
    mat_wood(0.54, 0.39, 0.19)
    draw_box(px + 0.010, 0.18, dz,
             0.030, panel_bot_h, fd - 0.14, 0.54, 0.39, 0.19)

    # Panel cermin (dari atas panel kayu sampai rail atas)
    mirror_bot = 0.18 + panel_bot_h + 0.02   # sedikit di atas panel kayu
    mirror_h   = (fw - 0.04) - mirror_bot     # sampai tepat di bawah rail atas
    mat_glass(0.72, 0.78, 0.82)
    draw_box(px + 0.018, mirror_bot, dz,
             0.015, mirror_h, fd - 0.14, 0.72, 0.78, 0.82)

    # Gagang — bar horizontal logam (di tengah pintu)
    handle_y = 0.18 + panel_bot_h * 0.5
    mat_metal(0.72, 0.68, 0.62)
    draw_box(px - 0.04, handle_y, dz, 0.022, 0.022, 0.18, 0.72, 0.68, 0.62)
    draw_box(px - 0.04, handle_y, dz - 0.09, 0.032, 0.032, 0.022, 0.65, 0.62, 0.58)
    draw_box(px - 0.04, handle_y, dz + 0.09, 0.032, 0.032, 0.022, 0.65, 0.62, 0.58)
...
def draw_bookshelf():
    ...
    # Dekorasi atas rak — semua dibatasi agar tidak melebihi sH + 0.22
    # Pot tanaman kecil
    set_material(0.62, 0.38, 0.22, amb_scale=0.25,
                 spec=(0.18,0.14,0.10,1.0), shininess=18.0)
    draw_box(sx + 0.02, sH + 0.01, sz + 0.60, 0.16, 0.14, 0.16,
             0.62, 0.38, 0.22)
    set_material(0.28, 0.54, 0.22, amb_scale=0.20,
                 spec=(0.04,0.07,0.03,1.0), shininess=6.0)
    draw_box(sx + 0.02, sH + 0.15, sz + 0.60, 0.15, 0.08, 0.15,
             0.28, 0.54, 0.22)

    # Vas kaca kecil
    mat_wood(0.40, 0.30, 0.18)
    draw_box(sx + 0.02, sH + 0.02, sz + 0.22, 0.06, 0.18, 0.20,
             0.40, 0.30, 0.18)
    mat_glass(0.66, 0.74, 0.80)
    draw_box(sx + 0.04, sH + 0.03, sz + 0.22, 0.02, 0.14, 0.14,
             0.66, 0.74, 0.80)

    # Lampu kecil
    set_material(0.92, 0.90, 0.85, amb_scale=0.30,
                 spec=(0.12,0.12,0.10,1.0), shininess=8.0)
    draw_box(sx + 0.02, sH + 0.02, sz - 0.30, 0.08, 0.12, 0.08,
             0.92, 0.90, 0.85)
    mat_emissive(1.00, 0.80, 0.28)
    draw_box(sx + 0.02, sH + 0.14, sz - 0.30, 0.015, 0.06, 0.015,
             1.0, 0.80, 0.28)
    mat_reset_emissive()

    # Buku flat di atas rak
